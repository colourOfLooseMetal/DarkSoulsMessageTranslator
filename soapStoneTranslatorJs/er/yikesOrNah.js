...
yikes_or_nah.buildAllowCases = function () {
  for(i = 0; i < yikes_or_nah.specialAllowCases.length; i++) {
...
const isYikes = async (
  ...
  if (params == 1) {
    params = [
      [
        ...
        0.98, //obscene
        0.6, //severeToxicity
        0.7, //sexuallyExplicit
        0.6, //threat
        0.95, //generalToxicity
      ]
    ];
  }
  ...
  if (contains) {
    console.log("contains a banned word/phrase, direct match");
    return;
  }
  ...
  for (i = 0; i < predictions.length; i++) {
    if (i == 6) {
      if (
        ...
        (predictions[1].results[0].probabilities[1] > 0.8 ||
          predictions[1].results[1].probabilities[1] > 0.8)
      ) {
        yikes = true;
        console.log(
          predictions[i].label,
  ...
  if (yikes == false) {
    make_meme(topText, bottomText, messageType);
  }
  /*
        prints:
        {
